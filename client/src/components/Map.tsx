...
import { GOOGLE_MAP_NAMESPACE_GRACE_MS } from "./googleMapsConfig";
...
const MAPS_SCRIPT_ID = "manus-google-maps-api";
...
function loadMapScript(): Promise<void> {
  ...
  mapsScriptPromise = new Promise<void>((resolve, reject) => {
    ...
    const complete = () => {
      ...
      const deadline = Date.now() + GOOGLE_MAP_NAMESPACE_GRACE_MS;
      const waitForNamespace = () => {
        if (isMapsReady()) {
          ...
          window.setTimeout(waitForNamespace, 50);
        }
      };
      waitForNamespace();
    };
